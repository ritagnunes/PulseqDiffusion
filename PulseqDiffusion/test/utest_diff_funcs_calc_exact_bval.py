...
class TestCalexactbval(unittest.TestCase):
    def test_calexacetbval1(self):
        path =  Path(__file__).parent / 'utest_sample_data' / 'calc_exact_bval1.pkl'
        with open(path, 'rb') as f:
            waveform, INV, short_f, seq = pickle.load(f)

        result = difunc.calc_exact_bval(waveform, INV, short_f, seq)
        self.assertAlmostEqual(result, 327.19201047729877)

    def test_calexacetbval2(self):
        path = Path(__file__).parent / 'utest_sample_data' / 'calc_exact_bval2.pkl'
        with open(path, 'rb') as f:
            waveform, INV, short_f, seq = pickle.load(f)

        result = difunc.calc_exact_bval(waveform, INV, short_f, seq)
        self.assertAlmostEqual(result, 498.775665378322)

    def test_calexacetbval3(self):
        path = Path(__file__).parent / 'utest_sample_data' / 'calc_exact_bval3.pkl'
        with open(path, 'rb') as f:
            waveform, INV, short_f, seq = pickle.load(f)

        result = difunc.calc_exact_bval(waveform, INV, short_f, seq)
        self.assertAlmostEqual(result, 749.6658744624788)
